...
function check_authorize($log, $pass)
{
    $users = get_users();
    if (array_key_exists($log, $users) && password_verify($pass, $users[$log]['pass'])) {
...
function add_user($login, $password)
{
    ...
    if (check_log($login)) {
        ...
        $users[$login] = ["pass" => password_hash($password, PASSWORD_DEFAULT), 'role' => 'user'];
        update_users($users);
        $_SESSION['authorized'] = 1;
...
{
    $users = get_users();
    return isset($_SESSION['authorized']) && $_SESSION['authorized'] == 1 && $users[$log]['role'] == 'admin';
}
function check_role($log, $pass)
{
    $users = get_users();
    return check_authorize($log, $pass) ? $users[$log]['role'] : false;
}
